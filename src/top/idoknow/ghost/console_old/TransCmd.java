    ...
    public void handleCommand(String typeCmd) throws IOException {
        try {
            ...
            switch (cmd[0]) {
                ...
                case "!list": {
                    Out.say("TransCmd-info", "已建立的连接(" + ConsoleMain.socketArrayList.size() + ")：\nindex\tname\tconnTime\tstate\tversion\tinstallTime");
                    for (HandleConn conn : ConsoleMain.socketArrayList) {
                        ...
                        Out.say(conn.rtIndex + "\t" +String.format("%-14s", conn.hostName) + "\t" + (d.getDate() + "," + d.getHours() + ":" + d.getMinutes() + ":" + d.getSeconds()) + "\t"
                                + (conn.equals(ConsoleMain.focusedConn) ? "聚焦" : "后台")+"\t"+conn.version+"\t"+conn.installTime);
                    }
                    Out.say("TransCmd-info", "列表完成.");
                    Out.say("TransCmd-info","Server启动于:"+ TimeUtil.millsToMMDDHHmmSS(ConsoleMain.serverStartTime));
                    ConsoleMain.cmdProcessFinish();
                    return;
                }
                case "!echo":{
                ...
                case "!pw": {
                    ConsoleMain.masterPw = cmd[1];
                    ConsoleMain.config.set("master.pw", cmd[1]);
                    ConsoleMain.config.write();
                    Out.say("TransCmd-pw", "master新密码:" + ConsoleMain.masterPw);
                    ConsoleMain.cmdProcessFinish();
                    return;
                }
                case "!dfocus": {
                    ConsoleMain.focusedConn = null;
                    Out.say("TransCmd-dfocus", "取消聚焦");
                    ConsoleMain.sendListToMaster();
                    ConsoleMain.cmdProcessFinish();
                    return;
                }
                case "!focus": {
                    ...
                    if (cmd[1].startsWith("&")) {
                        try {
                            ...
                            if(ConsoleMain.focusedConn!=null&& ConsoleMain.focusedConn.rtIndex==i)//已经聚焦到目标
                                return;
                            //Out.say("index:" + i);
                            for (HandleConn conn : ConsoleMain.socketArrayList) {
                                if (conn.rtIndex == i) {
                                    ConsoleMain.focusedConn = conn;
                                    Out.say("TransCmd-focus", "聚焦" + ConsoleMain.focusedConn.hostName);
                                    ConsoleMain.focusedConn.showHistory();
                        ...
                        if(ConsoleMain.focusedConn!=null&& ConsoleMain.focusedConn.hostName.startsWith(cmd[1])){//已经聚焦到目标
                            return;
                        }
                        for (HandleConn conn : ConsoleMain.socketArrayList) {
                    ...
                    if (cmd.length >= 3) {
                        for (HandleConn conn : ConsoleMain.socketArrayList) {
                            if (conn.hostName.startsWith(cmd[1])) {
                                ...
                                conn.bufferedWriter.newLine();
                                conn.bufferedWriter.flush();
                                ConsoleMain.tagLog.addTag(cmd[2],"login");
                ...
                case "!rmtag":{
                    ...
                    for(String owner: ConsoleMain.tagLog.getAllOwner().keySet()){
                    ...
                    if(delete.size()!=0){
                        for(String dk:delete){
                            ...
                            ConsoleMain.tagLog.getAllOwner().remove(dk);
                        }
                ...
                case "!svfile":{
                    switch (cmd[1]){
                        case "dir":{
                            String reply="!svDir "+ ConsoleMain.fileManager.getFileListString()+"!";
                            ConsoleMain.sendToSpecificMaster(reply,"serverFileExplorer");
                            break;
                        }
                        case "cd":{
                            if (cmd.length<3){
                                ConsoleMain.sendToSpecificMaster("ParamsNotEnough","serverFileExplorer");
                                break;
                            }
                            String reply= ConsoleMain.fileManager.changeDir(cmd[2]);
                            break;
                        }
                    }
                    return;
                ...
                case "!rft":{
                    ...
                    switch (cmd[1]){
                        case "chdir":{//修改文件保存根目录
                            ...
                            ConsoleMain.fileReceiver.setRootPath(cmd[2]);
                            Out.say("TransCmd-rft chdir","已修改文件服务器的保存根目录为"+cmd[2]);
                            ConsoleMain.cmdProcessFinish();
                            return;
                        }
                        case "dir":{
                            Out.say("TransCmd-rft dir","文件服务器的根目录是"+ ConsoleMain.fileReceiver.getRootPath());
                            ConsoleMain.cmdProcessFinish();
                            return;
                        }
                        case "task":{
                            try {
                                Map<String, BufferedFileReceiver.ReceiveTask> taskMap = ConsoleMain.fileReceiver.getTaskMap();
                                Out.say("TransCmd-rft task","遍历所有正在进行的文件接收任务\ntoken    fileName    length    received   savePath");
                                for (String token:taskMap.keySet()){
                                    Out.say(token+"   "+taskMap.get(token).getInfo().getName()+"   "+taskMap.get(token).getInfo().getSize()+"    "+taskMap.get(token).getReceivedSize()+"   "+ ConsoleMain.fileReceiver.getRootPath()+taskMap.get(token).getInfo().getSavePath());
                                }
...
//                                Out.say(token+"   "+taskMap.get(token).getInfo().getName()+"   "+taskMap.get(token).getInfo().getSize()+"    "+taskMap.get(token).getReceivedSize()+"   "+ServerMain.fileReceiver.getRootPath()+taskMap.get(token).getInfo().getSavePath());
                                if(token.startsWith(cmd[2])){
                                    ConsoleMain.fileReceiver.interruptFile(token);
                ...
                case "!jre":{
                    ...
                    switch (cmd[1]){
                         ...
                         */
                        case "view":{
                            ConsoleMain.jreRegister.sync();
                            Out.say("TransCmd-jre view","列表所有jre实体文件\nindex\tversion\tfilePath\tfileName\t    tag");
                            int index=0;
                            for(JRERegister.jreFile jreFile: ConsoleMain.jreRegister.files){
                        ...
                        case "reg":{
                            ...
                            for(String index:fileIndexs){
                                if(index.equals("all")){
                                    for(JRERegister.jreFile jreFile: ConsoleMain.jreRegister.getFiles()){
                                        jreFile.version=Long.parseLong(cmd[2]);
                                        Out.say(cmd[2]+"\t"+jreFile.filePath+"\t"+jreFile.fileName+"\t"+jreFile.tag);
                                    }
                                    break;
                                }
                                if(Integer.parseInt(index)>= ConsoleMain.jreRegister.getFiles().size()||Integer.parseInt(index)<0){
                                    Out.say("无index为"+index+"的文件");
                                    continue;
                                }
                                ConsoleMain.jreRegister.getFiles().get(Integer.parseInt(index)).version=Long.parseLong(cmd[2]);
                                Out.say(cmd[2]+"\t"+ ConsoleMain.jreRegister.getFiles().get(Integer.parseInt(index)).filePath+"\t"+ ConsoleMain.jreRegister.getFiles().get(Integer.parseInt(index)).fileName);
                            }
                            ConsoleMain.jreRegister.writeToFile();
                        ...
                        case "tag":{//为字段打标记
                            ...
                            for(String index:fileIndexs){
                                ...
                                ConsoleMain.jreRegister.getFiles().get(Integer.parseInt(index)).tag=cmd[2];
                                Out.say(cmd[2]+"\t"+ ConsoleMain.jreRegister.getFiles().get(Integer.parseInt(index)).filePath+"\t"+ ConsoleMain.jreRegister.getFiles().get(Integer.parseInt(index)).fileName);
                            }
                            ConsoleMain.jreRegister.writeToFile();
                        ...
                        case "rmtag":{
                            ...
                            for(String index:fileIndexs){
                                ...
                                ConsoleMain.jreRegister.getFiles().get(Integer.parseInt(index)).tag="";
                                Out.say("清除tag\t"+ ConsoleMain.jreRegister.getFiles().get(Integer.parseInt(index)).filePath+"\t"+ ConsoleMain.jreRegister.getFiles().get(Integer.parseInt(index)).fileName);
                            }
                            ConsoleMain.jreRegister.writeToFile();
                ...
                case "!note":{
                    ...
                    Out.say("note:"+note);
                    ConsoleMain.note=new StringBuffer(note);
                    ConsoleMain.cmdProcessFinish();
                    return;
                }
                case "!exit":{
                    ...
                    if(cmd[1].equalsIgnoreCase("-m")){
                        ...
                        kill.addAll(ConsoleMain.socketArrayList);
                        Out.say("退出所有客户端");
                    }else if(cmd[1].equalsIgnoreCase("-lv")){
                        Out.say("TransCmd-lv","退出低版本客户端");
                        i:for (int i = 0; i< ConsoleMain.socketArrayList.size(); i++){
                            HandleConn conn= ConsoleMain.socketArrayList.get(i);
                            try {
                                long ver0=Long.parseLong(conn.version.substring(1));
                                Out.sayThisLine(conn.hostName+":");
                                for (int j = 0; j< ConsoleMain.socketArrayList.size(); j++) {
                                    try {
                                        HandleConn conn1= ConsoleMain.socketArrayList.get(j);
            ...
            try {
                ConsoleMain.focusedConn.bufferedWriter.write(typeCmd);
                ConsoleMain.focusedConn.bufferedWriter.newLine();
                ConsoleMain.focusedConn.bufferedWriter.flush();
            } catch (Exception e) {
                e.printStackTrace();
                Out.say("TransCmd", "指令传输失败");
                ConsoleMain.killConn(ConsoleMain.focusedConn);
                ConsoleMain.cmdProcessFinish();
            }
        }catch (Exception e){
            throw e;
